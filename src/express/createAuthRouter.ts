...
import { handleDeviceList, handleDeviceRegister, handleDeviceRevoke } from '../devices/handlers.js';
import { loginWithPassword, registerWithPassword } from '../email/passwordLogin.js';
...
import { DEVICE_ID_HEADER, parseDeviceContext, type DevicePlatform } from '../types.js';
import { getClientIp, getUserAgent, parseCookies, parseFormBody, renderLoginPage } from './helpers.js';
...
function setSessionCookie(
  ...
  res.cookie(kit.config.cookie.name, signed, kit.sessions.getCookieOptions(maxAgeSeconds));
...
async function resolveAuthenticatedUserId(req: Request, kit: AuthKit): Promise<number | null> {
...
function sendError(res: Response, error: unknown): void {
  if (isAuthError(error)) {
    const body: Record<string, unknown> = { error: error.code, error_description: error.message };
...
function corsMiddleware(kit: AuthKit) {
  return async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    if (origin) {
      const allowed = await kit.oauthClients.getAllAllowedOrigins(kit.config.globalAllowedOrigins);
...
function csrfMiddleware(kit: AuthKit) {
  return (req: Request, res: Response, next: NextFunction): void => {
    ...
    if (hasSession && cookies.uids_csrf && !validateCsrf(req)) {
      res.status(403).json({ error: 'csrf_failed', error_description: 'CSRF validation failed' });
      return;
    }
    next();
  };
...
export function createAuthRouter(kit: AuthKit): Router {
  ...
  router.post('/token', async (req, res, next) => {
    try {
      const tokens = await handleTokenExchange(kit, req.body as Record<string, unknown>);
      res.json(tokens);
    } catch (error) {
      next(error);
    }
  ...
  router.post('/refresh', async (req, res, next) => {
    try {
      const tokens = await handleRefresh(kit, req.body as Record<string, unknown>);
      res.json(tokens);
    } catch (error) {
      next(error);
    }
  ...
  router.get('/oauth/google/start', async (req, res, next) => {
    try {
      ...
      const url = await startSocialLogin(kit, 'google', { pendingState: state });
      res.redirect(url);
    } catch (error) {
      next(error);
    }
  ...
  router.get('/oauth/google/callback', async (req, res, next) => {
    try {
      ...
      const session = await kit.sessions.createSession({ userId: result.userId });
  ...
  router.get('/oauth/microsoft/start', async (req, res, next) => {
    try {
      const state = typeof req.query.state === 'string' ? req.query.state : undefined;
      const url = await startSocialLogin(kit, 'microsoft', { pendingState: state });
      res.redirect(url);
    } catch (error) {
      next(error);
    }
  ...
  router.post('/email/password/login', async (req, res, next) => {
    try {
      const body = { ...parseFormBody(req.body), ...(req.body as object) } as Record<string, unknown>;
      const email = body.email;
      const password = body.password;
      if (typeof email !== 'string' || typeof password !== 'string') {
        res.status(400).json({ error: 'invalid_request' });
      ...
      const result = await loginWithPassword(kit, {
        ...
        pendingState: typeof body.pending_state === 'string' ? body.pending_state : undefined,
        ip: getClientIp(req),
        userAgent: getUserAgent(req),
        deviceId: deviceCtx.deviceId,
  ...
  router.post('/session/revoke', csrfMiddleware(kit), async (req, res, next) => {
    try {
      const sessionToken = getSessionToken(req, kit);
      if (!sessionToken) {
        res.status(401).json({ error: 'unauthorized' });
  ...
  router.post('/devices/revoke', csrfMiddleware(kit), async (req, res, next) => {
    try {
      const userId = await resolveAuthenticatedUserId(req, kit);
      if (!userId) {
        res.status(401).json({ error: 'unauthorized' });
      ...
      await handleDeviceRevoke(kit, userId, req.body as Record<string, unknown>);
      res.json({ success: true });
    } catch (error) {
      next(error);
    }
