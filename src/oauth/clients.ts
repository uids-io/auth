...
export function validateAuthorizeParams(query: Record<string, unknown>): {
  ...
  if (responseType !== 'code') {
    throw new InvalidRequestError('Unsupported response_type', 'unsupported_response_type');
  }
  ...
  if (codeChallengeMethod !== 'S256') {
    throw new InvalidRequestError('Unsupported code_challenge_method', 'invalid_request');
  }
  ...
  return {
    ...
    clientId,
    redirectUri,
    scopes: parseScope(typeof query.scope === 'string' ? query.scope : undefined),
