...
export async function startMagicLink(
  ...
  if (!kit.config.email.sendMagicLink) {
    throw new InvalidRequestError('Magic link not configured', 'magic_link_disabled');
  }
...
export async function consumeMagicLink(
  ...
  try {
    ...
    await client.query(`UPDATE auth.magic_links SET consumed_at = now() WHERE token_hash = $1`, [
      tokenHash,
    ]);
    ...
    if (row.client_id && row.redirect_uri && row.code_challenge && row.code_challenge_method === 'S256') {
