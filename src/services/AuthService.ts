...
export class AuthService {
	...
	async consumePendingContext(
		state: string,
	): Promise<PendingAuthContext | null> {
		...
		try {
         ...
         WHERE state = $1 FOR UPDATE`,
			...
			if (!rows[0] || new Date(rows[0].expires_at) <= new Date()) {
				await client.query(
					"DELETE FROM auth.pending_auth_context WHERE state = $1",
					[state],
				);
	...
	async cleanupExpired(): Promise<void> {
		await this.pool.query(
			`DELETE FROM auth.pending_auth_context WHERE expires_at <= now()`,
		);
		await this.pool.query(
			`DELETE FROM auth.oauth_authorization_codes WHERE expires_at <= now() AND consumed_at IS NULL`,
		);
		await this.pool.query(
			`DELETE FROM auth.magic_links WHERE expires_at <= now() AND consumed_at IS NULL`,
		);
	}
}
