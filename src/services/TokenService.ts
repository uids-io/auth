...
export class TokenService {
  ...
  async getActiveSigningKey(): Promise<{ kid: string; privateKey: CryptoKey; publicJwk: JWK }> {
  ...
  async issueTokens(params: {
    ...
    const client = await this.pool.query<{ access_token_ttl_seconds: number; refresh_token_ttl_seconds: number }>(
  ...
  async exchangeAuthorizationCode(params: {
    ...
    try {
      ...
      const row = rows[0];
      if (!row) {
        throw new InvalidRequestError('Invalid authorization code', 'invalid_grant');
      }
      if (row.consumed_at) {
        throw new InvalidRequestError('Authorization code already used', 'invalid_grant');
      }
      if (new Date(row.expires_at) <= new Date()) {
        throw new InvalidRequestError('Authorization code expired', 'invalid_grant');
      }
  ...
  async refreshAccessToken(refreshToken: string): Promise<{
    ...
    try {
      ...
      if (row.revoked_at || row.rotated_at) {
        await this.revokeSessionChain(client, Number(row.session_id));
        await client.query('COMMIT');
        throw new UnauthorizedError('Refresh token reuse detected', 'invalid_grant');
      }
      ...
      await client.query(
        ...
        [row.session_id, hashToken(newRefreshToken), row.id, refreshTtl],
      );
      ...
      const device: Device | null =
        row.device_external_id && row.platform
          ? {
              id: Number(row.device_id),
              deviceId: row.device_external_id,
              clientId,
              userId,
